...
import { callPaymentLink, paymentMethodLabel, getPaymentLinkSessionToken, type PublicPaymentLinkData } from '@/lib/paymentLinks';

export default function PublicPaymentLinkSuccess() {
  const navigate = useNavigate();
  const { token } = useParams<{ token: string }>();
  const [data, setData] = useState<PublicPaymentLinkData | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // token (stored at payment link open). Without one, send them to the
      // payment link page, which will open a fresh session for the link.
      const sessionToken = getPaymentLinkSessionToken(token);
      if (!sessionToken) {
        navigate(`/payment-link/${token}`, { replace: true });
        return;
      }
      const result = await callPaymentLink<PublicPaymentLinkData>('get-status', { token: sessionToken });
      if (result.not_found) {
              ...
              <div className="flex justify-between">
                ...
                <span className="inline-flex items-center gap-1.5 font-medium text-foreground">
                  {data.session.selected_payment_method
                    ? paymentMethodLabel(data.session.selected_payment_method)
                    : 'Online'}
                  {data.session.selected_payment_method &&

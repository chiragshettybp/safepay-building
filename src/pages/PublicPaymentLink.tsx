...
import { PaymentLinkSummary } from '@/components/shared/PaymentLinkSummary';
import {
...
import {
  ...
  paymentMethodLabel,
  PAYMENT_METHODS,
  getPaymentLinkSessionToken,
  setPaymentLinkSessionToken,
  PaymentLinkApiError,
...
export default function PublicPaymentLink() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await callPaymentLink<OpenLinkResponse>('open-link', {
        linkToken: token,
        sessionToken: getPaymentLinkSessionToken(token),
      });
      setData(result);
      if (result.not_found) {
        ...
        setPaymentLinkSessionToken(token, result.session.token);
      ...
      setLoadError(error instanceof PaymentLinkApiError ? error.message : 'Something went wrong while loading this payment link.');
    } finally {
      setIsLoading(false);
    }
  }, [token, navigate]);
  ...
  const handleContinue = () => {
    ...
    callPaymentLink<CreatePaymentResponse>('create-payment', {
      token: sessionToken,
      name: name.trim(),
      phone,
      ...
      .catch((error: unknown) => {
        if (error instanceof PaymentLinkApiError && error.code === 'SESSION_EXPIRED') {
          setTerminal('expired');
        } else if (error instanceof PaymentLinkApiError && error.code === 'SESSION_NOT_ACTIVE') {
          setTerminal('cancelled');
        } else {
  ...
  const handleVerify = (razorpayParams?: { paymentId: string; orderId: string; signature: string }) => {
    ...
    callPaymentLink<{ success: boolean }>('verify-payment', {
      token: sessionToken,
      ...
      .then(() => {
        navigate(`/payment-link/${token}/success`, { replace: true });
      })
      .catch((error: unknown) => {
        if (error instanceof PaymentLinkApiError && error.code === 'SESSION_EXPIRED') {
          setTerminal('expired');
  ...
  const handleCancelPayment = async () => {
    ...
    isSubmittingRef.current = true;
    try {
      await callPaymentLink('cancel-payment', { token: sessionToken, transactionId: payment.transactionId, reason: 'cancelled_by_customer' });
    } catch {
      ...
      {resumeNotice && (
        <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2.5 mb-4">
          ...
          <p className="text-xs text-amber-700">
            You have an unfinished payment on this link. Review your details and continue to resume payment.
          </p>
        </div>
      )}
      {/* Order summary */}
    ...
    <div className="flex-1 w-full max-w-md mx-auto px-4 pt-4 pb-32">
      <div className="bg-card rounded-2xl border border-border p-5 mb-4 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-3">
          {payment.mode === 'test'
            ...
            : `Pay via ${paymentMethodLabel(method)} through the secure gateway.`}
        </p>
      ...
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-2">
          <p className="text-xs font-medium text-foreground">Paying via {paymentMethodLabel(method)}</p>
